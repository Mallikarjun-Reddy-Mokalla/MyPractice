...
public class OverlayView extends View {
    ...
    private int customStartX, customStartY, customEndX, customEndY;

    public static final RectF mCropViewRect = new RectF();
    ...
    private String mTextCropRectToDraw;
    private String mTextCropRectToDrawwindow;

    public static RectF mCustomCropWindowRect = new RectF();
    public static RectF mCropViewRectNewRectF = new RectF();
    private Paint mCustomCropWindowPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    public void setTextToDraw(String text, String text2, String text3) {
        mTextToDraw = text + text2 + text3;
        invalidate(); // Force redraw of the view
    }

    public void setTextToDrawCrop(String text, String text2) {
        mTextCropRectToDraw = text;
        mTextCropRectToDrawwindow = text2;
        invalidate(); // Force redraw of the view
    }

    private void drawText(Canvas canvas) {
        if (mTextToDraw != null) {
            // Define paint settings for the text
            Paint textPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            textPaint.setColor(Color.WHITE);
            textPaint.setTextSize(30); // Adjust the text size as needed
            textPaint.setTextAlign(Paint.Align.CENTER);
            float x = mCropViewRect.centerX();
            float y = mCropViewRect.top + Math.abs(textPaint.ascent());

            // Draw the text on the canvas
            canvas.drawText(mTextToDraw, x, y, textPaint);
        }
    }

    private void drawTextCrop(Canvas canvas) {
        if (mTextCropRectToDraw != null) {
            // Define paint settings for the text
            Paint textPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            ...
            textPaint.setTextAlign(Paint.Align.CENTER);
            float x = mCropViewRect.centerX();
            float y = mCropViewRect.top + Math.abs(textPaint.ascent()) + 100;
            float y2 = mCropViewRect.top + Math.abs(textPaint.ascent()) + 150;

            // Draw the text on the canvas
            canvas.drawText(mTextCropRectToDraw, x, y, textPaint);
            canvas.drawText(mTextCropRectToDrawwindow, x, y2, textPaint);
        }
    }

    // Calculate the coordinates for the custom crop window
    private void calculateCustomCropWindow() {
        int displayWidth = getResources().getDisplayMetrics().widthPixels;
        int displayHeight = getResources().getDisplayMetrics().heightPixels;
        int cropWindowMargin = 100;

        int cropWidth = displayWidth - (2 * cropWindowMargin) + 50;
        int cropHeight = 300;

        int left = (displayWidth - cropWidth) / 2 + 75;
        int top = (displayHeight - cropHeight) / 3 - 50;
        int right = left + cropWidth - 150;
        int bottom = top + cropHeight;

        mCustomCropWindowRect.set(left, top, right, bottom);
        postInvalidate();
    }
...
/*
    public void setupCropBounds() {
        ...
        calculateCustomCropWindow(); // Call the method to calculate custom crop window
...
*/
// Inside the OverlayView class

    public void setupCropBounds() {
        int desiredWidth = 600;
        int desiredHeight = 600;

        int left = (mThisWidth - desiredWidth) / 2;
        int top = (mThisHeight - desiredHeight) / 2;
        int right = left + desiredWidth;
        int bottom = top + desiredHeight;
...
/*
    public void setupCropBounds() {
        ...
        int left = (screenWidth - desiredWidth) / 2;
        int top = (screenHeight - desiredHeight) / 2;
        int right = left + desiredWidth;
        int bottom = top + desiredHeight;
...
*/
    ...
    protected void init() {
        ...
        // Initialize custom crop window paint
        mCustomCropWindowPaint.setColor(Color.RED);
        mCustomCropWindowPaint.setStyle(Paint.Style.STROKE);
        mCustomCropWindowPaint.setStrokeWidth(5);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawCropGrid(canvas);
        drawCustomCropWindow(canvas); // Draw the custom crop window
        drawText(canvas);
        drawTextCrop(canvas);
    }

    private void drawCustomCropWindow(Canvas canvas) {
        // Make sure we have valid custom crop window coordinates
        if (mCustomCropWindowRect.isEmpty()) {
            return;
        }

        // Draw the custom crop window
//        canvas.drawRect(mCustomCropWindowRect, mCustomCropWindowPaint);
    }
...
//        }
    ...
    protected void drawCropGrid(@NonNull Canvas canvas) {
        ...
        if (mShowCropFrame) {
            mCropViewRectNewRectF = mCropViewRect;
        ...
        if (mFreestyleCropMode != FREESTYLE_CROP_MODE_DISABLE) {
            ...
            canvas.drawRect(customStartX, customStartY, customEndX, customEndY, mCropFramePaint);
